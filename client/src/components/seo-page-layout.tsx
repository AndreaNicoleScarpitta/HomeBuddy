...
export function SeoPageLayout({ children, slug, title, description }: SeoPageLayoutProps) {
  useEffect(() => {
    ...
    const canonicalUrl = `https://home-buddy.replit.app${window.location.pathname}`;
      ...
      <header className="fixed top-0 w-full z-50 bg-background/80 backdrop-blur-md border-b border-border/40">
        <nav className="container mx-auto px-6 h-16 flex justify-between items-center">
          <Link href="/" className="flex items-center gap-2 no-underline">
            <img src="/images/home-buddy-icon.png" alt="Home Buddy logo" className="h-8 w-8 rounded-lg object-cover" width="32" height="32" />
